...
gcloud compute ssh mg-spark-master \
  --command="cd /opt/spark && ./sbin/start-master.sh && SPARK_WORKER_INSTANCES=4 ./sbin/start-slave.sh -c 24 -m 155G spark://mg-spark-master:7077"
...
for ((i=0;i<NUM_SLAVES;i++)); do
  ...
  gcloud compute ssh mg-spark-slave-${i} \
    --command="cd /opt/spark && SPARK_WORKER_INSTANCES=4 ./sbin/start-slave.sh -c 24 -m 155G spark://mg-spark-master:7077"
done
