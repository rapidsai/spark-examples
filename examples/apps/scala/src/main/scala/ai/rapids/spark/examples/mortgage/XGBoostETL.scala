...
private object AcquisitionETL extends MortgageETL {
  ...
  def createAcquisition: this.type = {
    val names = NameMapping.names
  ...
  def cleanPrime(perfDF: DataFrame): this.type = {
    dataFrame = perfDF.join(dataFrame, Seq("loan_id", "quarter"), "left_outer").drop("quarter")
    this
  }
}
...
object XGBoostETL extends Mortgage {
  ...
  private lazy val allCols =
    categaryCols.map(c => (md5(col(c._1)) % 100).alias(c._1)) ++
    numericCols.map(c => col(c._1))
