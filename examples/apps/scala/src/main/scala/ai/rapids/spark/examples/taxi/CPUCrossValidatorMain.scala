...
object CPUCrossValidatorMain extends Taxi {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val xgbRegressionModel = if (xgboostArgs.isToTrain) {
      ...
      val regressor = new XGBoostRegressor(xgbParamFinal)
      ...
      val (model, _) = benchmark.time("CrossValidation") {
        cv.fit(datasets.head.get).bestModel.asInstanceOf[XGBoostRegressionModel]
      }
